...
# 创建全局OCR服务实例
class OCRWorkerPool:
    """多卡 OCR worker 池，按卡绑定实例并做请求分发。"""

    def __init__(self, npu_device_ids: List[int], **ocr_kwargs):
        device_ids = npu_device_ids or [ocr_kwargs.get('npu_device_id', 0)]
        self.npu_device_ids = list(dict.fromkeys(device_ids))
        self.workers = []
        self._rr_index = 0
        ...
        for device_id in self.npu_device_ids:
            worker_kwargs = dict(ocr_kwargs)
            worker_kwargs['npu_device_id'] = device_id
            server = OCRServer(**worker_kwargs)
            self.workers.append({
                'device_id': device_id,
                'server': server
            })

        self.device_info = ",".join(
            f"NPU-{worker['device_id']}" for worker in self.workers
        )

    def _next_worker(self) -> Dict:
        if not self.workers:
            raise RuntimeError("OCR worker pool is empty")
    ...
    def _run_on_worker(self, worker: Dict, func_name: str, *args, **kwargs) -> Dict:
    ...
    def process_batch_images(
        ...
        if not images:
            return {
                ...
                'format_output': format_output,
                'device_ids': self.npu_device_ids,
            }

        start_time = time.time()
        workers = self.workers[:min(len(self.workers), len(images))]
        method_used = 'optimized' if use_optimized else 'traditional'
        ...
        if len(workers) == 1:
            ...
            result['device_ids'] = [workers[0]['device_id']]
            return result

        buckets = [[] for _ in workers]
        for index, image in enumerate(images):
            bucket_idx = index % len(workers)
            buckets[bucket_idx].append((index, image))
        ...
        with ThreadPoolExecutor(max_workers=len(workers)) as executor:
            ...
            for worker, bucket in zip(workers, buckets):
                ...
                future_to_worker[future] = (worker, bucket)

            for future in as_completed(future_to_worker):
                worker, bucket = future_to_worker[future]
                sub_result = future.result()
                if not sub_result['success']:
                    return {
                        'success': False,
                        'error': sub_result.get('error', f"worker NPU-{worker['device_id']} failed"),
                        'processing_time': time.time() - start_time,
                        'image_count': len(images),
                        'total_text_count': 0,
                        'method_used': method_used,
                ...
                successful_workers.append(worker['device_id'])
        ...
        return {
            ...
            'image_count': len(images),
            'total_text_count': total_text_count,
            'method_used': f"{method_used}_multi_npu",
            'format_output': format_output,
            'device_ids': successful_workers
        }
